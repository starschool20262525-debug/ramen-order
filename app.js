...
function isKitchenPage() {return document.getElementById("order-list") !== null;}

// ステータス
function setStatus(text) {const element = document.getElementById("connection-status");
...
{ setStatus("通信部品を読み込めませんでした。"); return;}
  ...
  // 新しい注文
  channel.bind("new-order", function(data) {console.log("新しい注文:", data);
  ...
  // メニュー変更
  channel.bind("toggle-menu", function(data) { console.log("メニュー変更:", data);
    const item = menuData.find(item => item.id === data.id);

    if (!item) {return;}

    item.available = data.available;

    // 売り切れになった商品はお客さん側の選択数を0
    if (!item.available) {order[item.id] = 0;}
    if (isCustomerPage()) { renderCustomerMenu(); updateSummary(); }
    if (isKitchenPage()) { renderKitchenMenu(); } });}
...
{  hasItems = true;
  ...
  totalElement.innerText =`${total} 円`;}
...
function completeOrder(id) {
  ...
  kitchenOrders =kitchenOrders.map(orderData => {
      if ( String(orderData.id) === String(id)) 
{ return { ...orderData, status: "提供済み" };}
   return orderData;});
...
function saveKitchenOrders() {
  localStorage.setItem( "ramen-kitchen-orders",
  JSON.stringify(kitchenOrders) );}

function loadKitchenOrders() {
  try { kitchenOrders =JSON.parse(
          localStorage.getItem( "ramen-kitchen-orders") || "[]");}

 catch {kitchenOrders = [];}}

// 通知音
function playNotification() {
  try {const AudioContext =
        window.AudioContext ||
